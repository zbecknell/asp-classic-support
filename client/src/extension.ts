...
import { getHtmlVirtualContent, isInsideStyleRegion } from './embeddedSupport';
	...
	let clientOptions: LanguageClientOptions = {
		...
		middleware: {
			provideHover: async (document, position, token, next) => {
				...
				virtualDocumentContents.set(originalUri, getHtmlVirtualContent(document));

				const vdocUriString = `embedded-content://html/${encodeURIComponent(
					originalUri
				)}.html`;
				const vdocUri = Uri.parse(vdocUriString);
				let result = await commands.executeCommand<Hover[]>(
